...
func GetRoles(w http.ResponseWriter, r *http.Request) {
	...
	item, err := helpers.AddFunctionInPQueue(pqueue.PQueuesInstance(background.ParentContext(r.Context())).SuperPQueue, any(githubClient.FindType), []any{any(*request.Link)}, pqueue.HighPriority)
	if err != nil {
		background.Log(r).WithError(err).Errorf("failed to add function in pqueue")
		ape.RenderErr(w, problems.InternalError())
		return
